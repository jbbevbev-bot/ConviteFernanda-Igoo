...
function setupModals() {
  ...
  qa('[data-close-pix]').forEach(el => el.addEventListener('click', () => closeModal('pixModal')));
  ...
  host.innerHTML = gifts.map((gift, index) => `
    <article class="stack-item" data-gift-index="${index}">
      ...
      <div class="admin-form-grid two-columns">
        ...
        <label>Permitir pagamento com Cartão
          <input data-gift-field="cardEnabled" type="checkbox" ${gift.cardEnabled !== false ? 'checked' : ''} />
        </label>
        <label>Permitir pagamento por Pix
          <input data-gift-field="pixEnabled" type="checkbox" ${gift.pixEnabled ? 'checked' : ''} />
        </label>
...
function wireAdminActions() {
  ...
  q('#addGiftBtn')?.addEventListener('click', () => {
    state.config.gifts.push({ title: 'Novo presente', price: 0, description: '', imageUrl: '', imageQuery: '', icon: 'fa-gift', mercadoPagoLink: '', pixEnabled: true, cardEnabled: true });
    renderGiftAdminList();
  });
